...
class ThirdlawGuardrailConfigModelOptionalParams(BaseModel):
    ...
    streaming_buffer_until_moderated: Optional[bool] = Field(
        default=True,
        description="If true (default), the response is not sent to the client until ThirdLaw has moderated the response.",
    )
