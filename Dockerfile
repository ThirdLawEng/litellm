...
# Debian (glibc) rather than alpine: on musl builders, environments that force
# npm_config_libc=glibc (a common CI/cross-build setting) make npm install the
# gnu swc binding and skip the musl one, so `next build` dies with
# "__res_init: symbol not found" and a Turbopack-unsupported WASM fallback.
ARG UI_BUILD_IMAGE=node:24.19-bookworm-slim@sha256:a9f5f7c91a432850b2a8a7797adf5eadb6c733ceed61167806cee7ea7fbc29df
